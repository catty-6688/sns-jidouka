...
const agentJobs: Record<AgentId, string> = {
  ...
  buzz:
    "リサーチ内容を分解し、なぜ伸びたかを分析する。フック、共感、逆張り、体験、専門性、保存される理由、コメントが付きやすい余白に分けて、このアカウントに移せる型だけを抽出する。無料相談CTAよりも、AIへの興味と信頼を育てる型を優先する。",
  account:
    "このアカウント自身の勝ちパターンを分析する。投稿後の表示、いいね、返信、保存されそうな要素、AIに興味を持たれそうな切り口を見て、次に増やす表現と減らす表現を出す。数字が不足している場合は、記録テンプレを作る。",
  planning:
    "リサーチ、バズ分析、自アカ分析、体験データをもとに、7日分の投稿テーマを作る。立ち上げ初期はAIの使い方、発信の作業分解、投稿ネタ、時短、SNS苦手克服を中心にし、MLM集客疲れ、副業迷子、美容好きにも自然に届く流れにする。",
  writing:
    "1週間分のThreads投稿案を作るための執筆指示書を作る。参考投稿から移すのは構成とフックの型だけに限定し、言葉を写さない。無料相談に急に誘導せず、AI投稿で信頼を積む流れにする。各投稿に実体験を入れ、足りないところは【ここに体験】と示す。"
};
...
function validateBody(body: unknown): AgentRequest | null {
  ...
  return {
    ...
    recentOwnPosts: pickString(data.recentOwnPosts),
...
- CTA/導線: ${body.offer || "無料相談は週1〜2本だけ。通常投稿はAIの使い方、共感、保存、返信を目的にする。"}

使える実体験・思想:
${body.personalExperiences || "まだ不足。足りない箇所は【ここに体験】と示す。"}

最近の自分の投稿:
${body.recentOwnPosts || "未設定。未設定の場合は、どんな投稿を5〜10本貼るべきか具体的に指示する。"}
...
- 立ち上げ初期はAIについての投稿を中心にする。無料相談への誘導は控えめにする
- AI投稿は、投稿ネタ出し、文章のたたき台、プロフィール整理、リサーチ、キャプション改善、作業手順化など具体例を入れる
- 最近の自分の投稿がある場合、言葉選び、改行、語尾、絵文字量、テンションを分析して「この人らしさ」として抽出する
- 絵文字は最近の投稿の量に合わせる。未設定なら1投稿0〜2個まで自然に使う前提で設計する
- 誇大表現、断定、楽に稼げる表現は避ける
